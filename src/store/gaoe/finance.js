...
import {URI_HI_FINANCE, URI_HI_FINANCE_CHARGE_CANCEL} from '@/constants'
...
const actions = {
  getInfo ({commit, dispatch, state}, params = {page: 1}) {
    ...
    let apiUri = `${URI_HI_FINANCE}?offset=${offset}&limit=${state.limit}`
  ...
  cancelCharge ({commit}, id) {
    return api(URI_HI_FINANCE_CHARGE_CANCEL, {
      method: 'POST',
      body: {id}
    }).then((res) => {
