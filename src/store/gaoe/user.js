...
const actions = {
  ...
  getChartData ({ commit }, payload) { // 按天数、按指标类型取图表数据
    ...
    let url = `${URI_HI_REPORT}?action=chart&content=${content}&day_cnt=${dayCnt}&start_time=${startTime}&end_time=${endTime}`
  ...
  getTableData ({ commit }, payload) {
    ...
    let startTime = now - dayCnt * 86400
    let endTime = now - 86400
    let url = `${URI_HI_REPORT}?action=table&content=${content}&start_time=${startTime}&end_time=${endTime}`
  ...
  downloadReport (ctx, payload) {
    ...
    let startTime = now - dayCnt * 86400
    let endTime = now - 86400
    location.href = `${URI_HI_REPORT_DOWNLOAD}?start_time=${startTime}&end_time=${endTime}`
  },
